...
доставки СДЭК, Деловых Линий и Magic Trans по данным из Google Таблицы
(артикулы, габариты, вес, правила групповой упаковки и составные комплекты).
...
3. Менеджер ищет товары по артикулу/наименованию, добавляет позиции с
   ...
   упаковки из таблицы.
4. Менеджер указывает города «откуда»/«куда», режим «склад/дверь» и при
   необходимости терминалы — сервер вызывает API выбранной ТК и показывает
   стоимость и срок доставки.

### Логика упаковки

- Если артикул описан на листе **«Составные артикулы»**, каждая единица
  комплекта разворачивается в фиксированные физические места. ДШВ и вес каждого
  места берутся из обычного справочника по артикулу места; групповые правила к
  этим компонентам не применяются.
...
- Серверные модули защищены `server-only` — импорт из клиентского кода
  ломает сборку.
- Закрытая страница `/admin/diagnostics` и её API проверяют зашифрованную
  Google-сессию на сервере и доступны только `rop.uwz@gmail.com`. Обычный
  health endpoint по-прежнему возвращает только `ok/down` без причин ошибок.
- Диагностические события содержат только безопасные коды, этапы, HTTP-статусы,
  задержку и request ID. Токены, заголовки авторизации, тела ответов и email
  редактируются или вообще не попадают в события.
...
| `MAGIC_TRANS_REFRESH_TOKEN` | Боевой refresh-token Magic Trans (только сервер) |
| `MAGIC_TRANS_SECURITY_API_URL` | Необязательно; по умолчанию production Security API Magic Trans |
| `MAGIC_TRANS_ECOMM_API_URL` | Необязательно; по умолчанию production Ecomm API Magic Trans |
| `DADATA_API_KEY` | Необязательно; подсказки улиц для адресов СДЭК (dadata.ru, бесплатно до 10 000 запросов/сутки). Без ключа улица — свободный текст |
...
```bash
npm install
npm test         # юнит-тесты парсинга, упаковки и Magic Trans
...
| `/admin/diagnostics` | Закрытая диагностика перевозчиков только для `rop.uwz@gmail.com`: этап и код ошибки, HTTP-статус, задержка, request ID и последние безопасные события |
...
- **Составные артикулы**: отдельный лист с колонками «Артикул комплекта»,
  «№ места», «Артикул места», «Количество мест». Номер места внутри одного
  комплекта должен быть уникальным, количество — положительным целым числом.
  Артикул места должен быть физическим товаром из справочника; вложенные
  составные комплекты отклоняются с ошибкой качества данных.
...
4. Под `rop.uwz@gmail.com` открывается `/admin/diagnostics`, а под другой
   учётной записью страница возвращает 404.
5. Расчёт доставки возвращает тарифы СДЭК.
...
app/
  ...
  admin/diagnostics/             — закрытая диагностика перевозчиков
  api/admin/diagnostics/         — server-only проверки и безопасные события
...
lib/
  ...
  adminPolicy.ts                 — точное правило доступа к диагностике
  adminAuth.ts                   — server-only guard диагностической админки
  carrierDiagnostics.ts          — безопасные типы, коды, редактура и буфер
  runCarrierDiagnostics.ts       — проверки ТК и структурированные события
