...
- Для разрешённой учётной записи заметная кнопка «Диагностика ошибок»
  показывается сразу под данными пользователя, в том числе при сбое доступа к
  Google Таблице. Остальным пользователям кнопка не рендерится.
- Проверка статусов на главной записывает полную безопасную причину каждого
  сбоя в закрытую историю диагностики. Публичный health-ответ и обычный
  интерфейс по-прежнему раскрывают только `ok/down`.
